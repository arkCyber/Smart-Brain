...
fn main() {
    ...
    if let Ok(s) = sub.try_recv() {
        println!("subscribed: {} = {}", s.key, String::from_utf8_lossy(&s.value));
    }
    ...
    let handler: QueryHandler =
        Arc::new(|key: &str| vec![format!("computed:{key}").into_bytes()]);
    let _handle = z.declare_queryable("service/distance", handler).unwrap();
    println!("compute replies = {}", z.get("service/distance").unwrap().len());
}
